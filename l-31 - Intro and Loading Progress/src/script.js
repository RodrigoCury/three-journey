...
loadManager.onProgress = (url, done, total) => {
    loadBar.style.transform = `scaleX(${done / total})`
}
loadManager.onLoad = () => {
    gsap.delayedCall(1, () => {
        gsap.to(
        ...
        loadBar.classList.add('ended')
        loadBar.style.transform = ``
    })
}
loadManager.onError = (url) => console.error(url)
 ...
 * Lights
 */
const directionalLight = new THREE.DirectionalLight('#ff0000', 3)
